            ...
            SettingsGroup(
                ...
                settings=[
                    Setting(
                        ...
                        type="string",
                        default=op.join(
                            op.expanduser("~"), ".local", "share", "snapcheck"
                        ),
                    ),
                ],
            ),
...
def load_settings(settings_f) -> Settings:
    if not op.isfile(settings_f):
        generate_default_settings(settings_f)
    return Settings.from_json(settings_f)
